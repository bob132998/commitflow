...
Siap bantu insight lebih cerdas. 💡`;
    ...
    useEffect(() => {
        const interval = setInterval(() => setPlaceholder(getRandomPlaceholder()), 5000);
        ...
        >
            ...
            <div className="flex justify-between items-center bg-[#1e293b] hover:bg-[#27364a] transition text-white p-2 md:rounded-t-lg cursor-pointer">
                <span className="flex gap-2 items-center">
                    ...
                    <span className="mt-1 text-sm text-cyan-400" aria-label={packageJson.version}>v{packageJson.version}</span>
                    <button onClick={onOffSound} className="py-1 px-2 rounded-sm ml-2 hover:bg-[#334155] transition">
                        {!isPlaySound ? <VolumeX className="w-5 h-5 text-gray-300" /> : <Volume2 className="w-5 h-5 text-cyan-400" />}
                    </button>
                    <button onClick={clearChat} className="flex gap-2 py-1 px-2 rounded-sm hover:bg-[#ef4444]/70 transition">
                        <Trash2 className="w-5 h-5 text-gray-300" />
                    </button>
                </span>
                <button onClick={closeWindow} className="py-1 px-2 rounded-sm hover:bg-[#334155] transition">
                    <X className="w-5 h-5 text-gray-300" />
                </button>
            </div>

            {/* Chat messages */}
            <div ref={containerRef} className="flex-1 p-4 overflow-y-auto space-y-2 bg-[#0f172a] pb-20">
                <motion.span
                    key={uuidv4()}
                ...
                >
                    <ReactMarkdown
                        ...
                        components={{
                            a: (props) => <a {...props} className="text-cyan-400 underline" target="_blank" rel="noopener noreferrer" />,
                            code: ({ inline, className, children, ...props }: any) => (
                                <code
                                    className={`cursor-pointer ${inline ? 'bg-[#1e293b] text-pink-400 rounded px-1' : 'block bg-[#1e293b] text-cyan-300 p-2 rounded-md my-2 overflow-x-auto'} ${className || ''}`}
                                    {...props}
                            ...
                            strong: ({ children }) => (
                                <strong className="text-emerald-400 drop-shadow-[0_0_6px_rgba(16,185,129,0.4)] font-semibold">{children}</strong>
                            ),
                            em: ({ children }) => <em className="text-blue-300 italic opacity-80">{children}</em>,
                        }}
                    >
                        {welcomeMessage || ""}
                    </ReactMarkdown>
                ...
                {(messages && messages.length > 0) && messages.map((msg: any) => {
                    ...
                    return (
                        <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`relative px-3 py-2 my-2 rounded-lg max-w-[90%] break-words ${msg.role === 'user'
                                ...
                                <ReactMarkdown
                                    ...
                                    components={{
                                        ...
                                        code: ({ inline, className, children, ...props }: any) => (
                                            <code
                                                className={`cursor-pointer ${inline ? 'bg-[#0f172a] text-pink-400 rounded px-1' : 'block bg-[#27364a] text-cyan-300 p-2 rounded-md my-2 overflow-x-auto'} ${className || ''}`}
                                                {...props}
                                                onClick={() => handleCopy(children)}
                                            >
                                                {children}
                                ...
                                {msg.role !== "user" && (
                                    <>
                                        ...
                                        <button onClick={() => shareTo("whatsapp", msg.content)} className="text-gray-400 hover:text-cyan-400 right-10 bottom-2 absolute">
                                            {shared ? <Check size={16} className="text-green-500" /> : <Share2 size={16} />}
                                        </button>
                                        <button onClick={() => handleCopy(msg.content)} className="text-gray-400 hover:text-cyan-400 right-2 bottom-2 absolute">
            ...
            <ChatInput input={input} setInput={setInput} handleSend={handleSend} isLoading={isLoading} placeholder={placeholder} playSound={playSound} />
        </div>
    );
}
